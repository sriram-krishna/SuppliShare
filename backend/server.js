...
// Endpoint to get a list of items sorted by dateposted in ascending order
app.get('/items/sort/dateposted/ascending', async (req, res) => {
  try {
    const client = await pool.connect();
    const query = 'SELECT ItemType, Description, zipcode, ItemPictureURL FROM Items ORDER BY dateposted ASC';
    const { rows } = await client.query(query);
...
app.get('/items/sort/dateposted/descending', async (req, res) => {
  try {
    const client = await pool.connect();
    const query = 'SELECT ItemType, Description, zipcode, ItemPictureURL FROM Items ORDER BY dateposted DESC';
    const { rows } = await client.query(query);
